...
if ($profile) {
    ...
    // Fetch associated promos
    $stmt = $pdo->prepare('SELECT promo_id FROM profile_promos WHERE profile_id = :profile_id');
        ...
        <form action="<?php echo htmlspecialchars($_SERVER['REQUEST_URI']); ?>" method="post">
            ...
            <div class="form-group">
                ...
                <div class="checkbox-group">
                    ...
                    $all_promos = $pdo->query($sql)->fetchAll();
                    foreach ($all_promos as $promo) {
                        $checked = in_array($promo['id'], $associated_promo_ids) ? 'checked' : '';
                        echo '<div class="checkbox">';
                        echo '<label>';
                        echo '<input type="checkbox" name="promo_ids[]" value="' . $promo['id'] . '" ' . $checked . '> ';
